...
public class WordList implements Iterable<String> {
	...
	/**
	 * Node of LinkedList that stores the item and a single reference to the next node.
	 * 
	 */
	private class Node {
		private String item;
	...
	public int indexOf(String key) {		
		...
		while (current != null){
			if(current.item.equals(key))
				return index;
			else{  //else section not needed, show to make it easy to read
	...
	private class WordListIterator implements Iterator<String> {
		...
		@Override
		public String next() {
			String item = current.item; //must save the current item before changing it
			current = current.next;
			return item;
		}
	...
	public static void main(String[] args) {
		...
		//use iterator to access all elements of the word list
		for(String el : list){
			System.out.print(el + "..");
		}
		System.out.println();
